...
``` ruby
  arr = ['10', '11', '9', '7', '8']
  arr.sort do { |num1, num2| num2.to_i <=> num1.to_i }
```
...
munsters.select { |k, v| v['gender'] == 'male' }.reduce do |memo, obj|
  memo + obj['age']
end

males = munsters.select { |name, info| info['gender'] == 'male' }
ages = males.map { |name, info| info['age'] }
ages.reduce(:+)
```
...
hsh.map do |k, v|
  if v[:type] == 'fruit'
    v[:colors].map { |element| element.capitalize }
  else
    v[:size].upcase
  end
end
...
arr.reject do |hash|
  hash.values.flatten.any? { |num| num.odd? }
end
```
...
def uuid
  id = UUID_FORMAT.map do |size|
    string = ''
    size.times { string << HEX_CHARS.sample }
    string
  end
