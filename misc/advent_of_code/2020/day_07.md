...
## Day 7 - Handy Haversacks - 12/7/2020
...
```javascript
// Input: string representing a list of baggage rules
// Output: a number representing a count of distinct bags that can eventually contain your bag (shiny gold)
// Rules/Implicit Knowledge
//   your bag is a shiny gold bag
//   output (distinct count) should include:
//     bags that can directly hold your shiny gold bag, as well as
//     "ancestor" bags that can eventually hold your shiny gold bag (inside of other bags)
//   baggage rules batch input
//     baggage rules on separate lines
//   bag rule
//     starts with container type (adjective, color, 'bags contain')
//     ends with 'no other bags' OR list of one or more contained types (number, adjective, color, 'bag' or 'bags')
//   it LOOKS LIKE every bag type is two words (an adjective and a color) - this needs to be verified
//   is one bag type completely described on a single line or can more than one line describe the same bag type?
//   can a bag type be undescribed, i.e. can it be listed as being containable but not specified whether it can contain anything or nothing?
//   I can possibly avoid answering some of these questions if I work backwards, starting with shiny gold bags
//   It looks like I can assume there are no endless loops (i.e. bag types that contain each other)
// Examples: see below
// Edge Cases: see questions under Rules
// Data Structure
//   input: single string; split on '\n' to array of rules (strings)
//   parse baggage rules to object: keys are container type names, values are objects of contained types
//     keys of contained type objects are contained type names, values are number (how many are contained)
//     consider the provided example (also listed below):
//     {
...
//     }
// Algorithm
//   parse batch of baggage rules to object
//     initialize baggageRules object
//     split string baggageRulesBatch on newlines to array of rule strings and iterate
//       container type is first two words in the string
//       contained types list is after 'contain ' (split string on contain and take the last part)
//         if list string is 'no other bags', continue to next iteration
//         initialize a contained object
//         split list string on ', ' and iterate
//           use regexp to match count (one or more digits at the start of the string)
//           use regexp to match type ((word word) bag|bags)
//           add contained type to contained object: contained[type] = count
//         add contained object to baggageRules object: baggageRules[containerType] = contained
//     return baggageRules object
//   work backwards from 'shiny gold':
//     initialize ancestors array
//     find all 'shiny gold' parents
//       initialize parents array
//       iterate baggageRules keys
//         if current baggage rule (baggageRules[currentKey]) has 'shiny gold' as a key
//           add it to parents array if it is not already included there
//     iterate parents array
//       add parent to ancestors if it is not already included there
//     initialize parents array (bag types that can directly hold shiny gold bag)
//     find all bag types that can directly contain a shiny gold bag (find all parents)
//       iterate baggageRules keys
//         if current baggage rule (baggageRules[currentKey]) has 'shiny gold' as a key
//           add bag type of current baggage rule to parents array
//     then find all bag types that can contain those bag types, etc, until the bag type is no longer contained
//       initialize ancestors array to copy of parents array
//       iterate parents array
//         find parents of current parent
//         if ancestors does not include new parent, add new parent to ancestors
// Code

// I ended up deviating from my algorithm quite a bit and solving this while coding.
// I had a tough time wrapping my head around when to stop (after finding all ancestors) without
// seeing my data structures in action.

let parse = function parse(baggageRulesBatch) {
  let baggageRules = {};

  baggageRulesBatch.split('\n').forEach(rule => {
    let containerType = rule.match(/^([a-z]+ [a-z]+)/)[0];
    let containedList = rule.split('contain ')[1];
    if (/no other bags?/.test(containedList)) return;

    let contained = {};
    containedList.split(', ').forEach(containedRule => {
      let count = Number(containedRule.match(/^\d+/));
      let type = containedRule.match(/[a-z]+ [a-z]+/)[0];
      contained[type] = count;
    });

    baggageRules[containerType] = contained;
  });

  return baggageRules;
};

let getParents = function getParents(bagType, baggageRules) {
  let parents = [];

  for (let container in baggageRules) {
    if (bagType in baggageRules[container]) {
...
let getAncestors = function getAncestors(bagType, baggageRules) {
  let ancestors = [];

  let currentParents = getParents(bagType, baggageRules);
  ...
  while (currentParents.length > 0) {
    ...
    currentParents.forEach(parent => {
      ...
      getParents(parent, baggageRules).forEach(grandparent => {
        if (!grandparents.includes(grandparent)) grandparents.push(grandparent);
      });
    });
...
let countDistinctContainersOf = function countDistinctContainersOf(bagType, baggageRulesBatch) {
  let baggageRules = parse(baggageRulesBatch);

  return getAncestors(bagType, baggageRules).length;
};

// let baggageRulesBatch =
// 'light red bags contain 1 bright white bag, 2 muted yellow bags.\n' +
// 'dark orange bags contain 3 bright white bags, 4 muted yellow bags.\n' +
// 'bright white bags contain 1 shiny gold bag.\n' +
// 'muted yellow bags contain 2 shiny gold bags, 9 faded blue bags.\n' +
...
// let baggageRules = parse(baggageRulesBatch);
// console.log(getParents('shiny gold', baggageRules));
// console.log(getAncestors('shiny gold', baggageRules));
console.log(countDistinctContainersOf('shiny gold', baggageRulesBatch) === 4);
```
...
```javascript
// The approach is somewhat similar, but little code can be re-used:
// instead of finding ancestors, we're now looking for descendents
// parse the baggageRulesBatch into an object as before
// find all children of shiny gold bag
//   children/descendents needs to be an object instead of array, because we need to track counts too

let getChildren = function getChildren(bagType, baggageRules) {
  return baggageRules[bagType] || {};
};

let countDescendants = function countDescendants(bagType, baggageRules) {
  let children = getChildren(bagType, baggageRules);
  if (Object.keys(children).length === 0) return 1;

  let sum = 0;
  for (let child in children) {
    let quantity = children[child];
    sum += quantity * countDescendants(child, baggageRules);
  }
...
let baggageRules = parse(baggageRulesBatch);
// countDescendants is inclusive - the original bag, shiny gold in this case, is included
// in the count; subtract one from final count accordingly
console.log(countDescendants('shiny gold', baggageRules) - 1);
```
