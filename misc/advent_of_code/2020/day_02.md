...
## Day 2 - Password Philosophy - 12/3/2020
...
```javascript
// Input: list of password policies and passwords (array of strings)
// Output: number representing the count of valid passwords
// Rules/Implicit Knowledge
//   one string contains a password policy and a password, delimited by a colon and space `: `
//   a policy contains a range and a character separated by a space
//     a range is two numbers separated by a dash
//   a valid password has the specified number of charaters
//     number specified by policy range: at least the minimum, at most the maximum (inclusive)
// Examples (see below)
// Edge Cases
//   does case matter? based on input, it looks like everything is lower case
// Data Structure
//   input: array strings
//   policy and password can be processed as strings
// Algorithm
//   map the array of strings
//     return whether or not the password is valid
//       parse input string (separate function)
//         separate string to policy and password (split at `: `)
//         separate policy to range and character (split at space)
//         separate range to min and max (split at -) and convert to numbers
//       count number of specified character in password (String.prototype.match)
//       if result is null, set count to 0
//       return whether or not count is within the range
//         greater than or equal to min and less than or equal to max
//   reduce to a sum of valid passwords and return it (initialize sum to 0)
//     if current element is true, return sum + 1
//     else return sum
// Code

let parseLine = function parseLine(line) {
  let [policy, password] = line.split(': ');
  let [range, character] = policy.split(' ');
  let [min, max] = range.split('-');

  return [password, character, Number(min), Number(max)];
};

let countValidPasswords = function countValidPasswords(list) {
  return list.map(line => {
    let [password, character, min, max] = parseLine(line);

    let charMatcher = new RegExp(`${character}`, 'g');
    let charCount = password.match(charMatcher);
    if (charCount === null) {
      charCount = 0;
    } else {
  ...
  }).reduce((sum, isValid) => (isValid ? sum + 1 : sum), 0);
};
```
...
// Rules/Implicit Knowledge
//   one string contains a password policy and a password, delimited by a colon and space `: `
//   a policy contains a position list and a character separated by a space
//     a position list is two positions separated by a dash
//     positions start with ONE - they are NOT zero indexed
//   a valid password has the specified charater in exactly one of the two listed positions
//     the specified character may freely appear elsewhere in the string
// Examples (see below)
// Edge Cases
//   does case matter? based on input, it looks like everything is lower case
// Data Structure
...
//       parse input string (separate function)
//         separate string to policy and password (split at `: `)
//         separate policy to position list and character (split at space)
//         separate position list to position1 and position2 (split at -) and convert to numbers
//       return whether (position1 matches and position2 does not) OR (position2 matches and position1 does not)
//   reduce to a sum of valid passwords and return it (initialize sum to 0)
//     if current element is true, return sum + 1
//     else return sum
// Code
...
let parseLine = function parseLine(line) {
  let [policy, password] = line.split(': ');
  let [positionList, character] = policy.split(' ');
  let [position1, position2] = positionList.split('-');

  return [password, character, Number(position1), Number(position2)];
};

let countValidPasswords = function countValidPasswords(list) {
  return list.map(line => {
    let [password, character, position1, position2] = parseLine(line);
